...
async def morning_weather(stopper, vk):
    ...
    if 0 > s.total_seconds() > -60:
        ...
        for u in usrs:
            ...
            values = {"user_id": u.uid, "message": await get_weather(city, 0)}
...
async def weather(msg, args):
    await db.get_or_create(Weather, uid=msg.user_id)
...
async def weather(msg, args):
    me = await db.execute(Weather.delete().where(Weather.uid == msg.user_id))
